...
import * as CommandFeedbackLocal from '@deepseek-ai/dsh-command-feedback-local'
...
function agent(ctx: Context): Agent {
  ...
  const id = SessionId('feedback-local-loader-agent')
...
describe('/feedback local real Loader composition through cordis.yml', () => {
  it('boots cordis.yml and records local feedback without model-visible output', async () => {
    root = await mkdtemp(join(tmpdir(), 'dsh-command-feedback-local-loader-'))
    ...
    await writeFile(configPath, [
      ...
      "- name: '@deepseek-ai/dsh-command-feedback-local'",
      '',
    ].join('\n'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@deepseek-ai/dsh-command-feedback-local', CommandFeedbackLocal],
    ])
