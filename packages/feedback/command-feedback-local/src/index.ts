 ...
 * @module @deepseek-ai/dsh-command-feedback-local
 */
...
import { getOrCreateSessionAnonymousUserId } from '@deepseek-ai/dsh-session-anonymous-user-id'
...
function executeFeedbackCommand(invocation: CommandInvocation, ctx: Context): CommandResult {
  ...
  const id = getOrCreateSessionAnonymousUserId(invocation.agent.session.id)
  return {
    kind: 'success',
    text: `Feedback recorded for session ${invocation.agent.session.id}\nAnonymous user: ${id}. ${sharingDisclosure(telemetry)}`,
  }
}
