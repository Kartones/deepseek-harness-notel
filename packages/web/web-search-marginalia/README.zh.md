---
description: "面向 ctx.web 的可选 Marginalia 搜索提供方：部署如何安装、选择并配置它，以及一次搜索会返回什么。"
...
`dsh-web-search-marginalia` 是由 [Marginalia Search](https://about.marginalia-search.com/article/api/) 支持的 `WebSearchProvider`，用于 harness [web 能力 seam](../web/README.zh.md)（`ctx.web`）。它发送带有查询字符串和 `API-Key` 标头的 `GET {baseURL}/search`，再将 Marginalia 的 JSON `results[]` 映射为 seam 规范化的 `WebSearchResult`。此包将 `marginalia` 搜索提供方注册到 `ctx.web`，通过可选的 `ctx.credentials` 服务为每次搜索解析凭据，并在存在发起请求的 Agent 会话时记录不含密钥的请求。它不注册面向模型的工具，也不依赖 `ctx.llm`。此包需要显式启用：安装它不会改变随附的 `deepseek-official` 选择。
...
通过 [`dsh-tool-web`](../tool-web/README.zh.md)，会话模型会看到返回的 URL、标题和 snippet。提供方不提供答案文本或日期。错误包装由消费方负责。
