...
DeepSeek 请求身份独立于应用归因。凭据解析成功后，每个提供方请求都会携带 `x-deepseek-harness-user-id`：携带相同 `GenerateOptions.sessionId` 的请求共享来自 [`@deepseek-ai/dsh-session-anonymous-user-id`](../../identity/session-anonymous-user-id/README.md) 的进程本地 id，而不带会话的直接调用会为该请求取得新的 id。该 id 不会跨会话、进程、重启或 harness home 持久化。携带 `GenerateOptions.sessionId` 的请求还会通过 `x-deepseek-harness-session-id` 发送该确切值；无会话调用会省略该标头。两个标头都会发送至解析后的 `baseURL`（包括已配置的 gateway），且不会进入请求正文或模型可见内容。
